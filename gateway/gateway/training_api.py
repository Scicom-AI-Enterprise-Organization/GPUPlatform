...
class CreateTrainingRunRequest(BaseModel):
    ...
    # on the decoder layers (dynamic avoids per-length recompiles; custom attention/SSM/flash-attn
    # kernels graph-break, the rest fuses). ASR/Whisper: HF Trainer whole-model compile (inductor).
    # Off by default — opt-in / experimental: smoke-test + check the recompiles log before relying on it.
...
async def _create_and_launch_run(
    ...
    config = {
        ...
        "torch_compile": bool(body.torch_compile),  # all tasks: torch.compile (LLM/TTS per-block, ASR HF Trainer)
