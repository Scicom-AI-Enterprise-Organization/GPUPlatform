    ...
    for u in candidates:
        ...
        while True:
            ...
            if r.status_code < 400:
                # RTF — the clip's own duration if the upstream reported one (verbose_json),
                # else measured off the upload we forwarded.
                dur = _body_duration_s(body) or _audio_duration_s(file_bytes, content_type, file_name)
                res.update(_record_rtf(endpoint_id, alias, "stt", dur, lat / 1000.0))
            return res
    raise HTTPException(status_code=502, detail={"error": f"all upstreams failed: {last_err}"})
...
async def _unary(app, request: Request, request_id: str, forward,
    ...
    async def work() -> Response:
        ...
        try:
            ...
            # Audio forwards (STT/TTS) also report their real-time factor — measured
            # duration + RTF + RTFx; absent for non-audio and for unmeasurable bodies.
            hdrs.update(_rtf_headers(res))
...
# RTF = processing wall seconds / audio second, for both audio directions: how long the
# upstream took per second of audio it transcribed (STT) or synthesized (TTS). < 1 =
# faster than real time. Reported on the response (X-RTF / X-RTFX / X-Audio-Seconds) and
# recorded to Prometheus. It needs the AUDIO DURATION, which we measure header-only — no
# decoder in the request path:
#   STT — the upstream's own `duration` (whisper verbose_json, which the proxy already
#         always asks for), else the uploaded clip's WAV/PCM header.
#   TTS — the generated bytes (this engine is 16-bit mono PCM at _TTS_PCM_SR, optionally
#         behind a WAV header).
# A compressed body (mp3/opus/flac/m4a) is unmeasurable that way, so those requests simply
# carry no RTF — never a guessed one.
_PCM_CONTENT_TYPES = ("audio/pcm", "audio/l16", "audio/x-pcm", "audio/basic",
                      "application/octet-stream", "")
_WAV_CONTENT_TYPES = ("audio/wav", "audio/x-wav", "audio/wave", "audio/vnd.wave")


def _base_ct(content_type: str) -> str:
    """`audio/wav; codecs=1` → `audio/wav` (lowercased, parameters dropped)."""
    return (content_type or "").split(";")[0].strip().lower()


def _is_compressed_audio(data: bytes) -> bool:
    """True if `data` opens with a compressed container's magic bytes (mp3/ogg/flac/mp4/
    aiff/matroska). Callers upload those with `application/octet-stream` and a bare
    filename often enough that content-type alone can't gate the raw-PCM assumption —
    and a compressed file's byte length says nothing about its duration. A raw-PCM clip
    whose first sample happens to look like an mp3 frame sync loses its RTF; a missing
    RTF is the safe failure, a fabricated one is not."""
    if len(data) < 12:
        return False
    if data[:3] == b"ID3" or (data[0] == 0xFF and data[1] & 0xE0 == 0xE0):     # mp3
        return True
    return (data[:4] in (b"OggS", b"fLaC", b"FORM", b".snd", b"\x1a\x45\xdf\xa3")
            or data[4:8] == b"ftyp")                                          # m4a/mp4


def _audio_duration_s(data: bytes, content_type: str = "", filename: str = "") -> Optional[float]:
    """Seconds of audio in `data`, from its container header alone. RIFF/WAVE is read from
    its fmt chunk (any rate/width/channel count); a headerless body is treated as the TTS
    engine's raw PCM only when its content-type/extension says so and its bytes don't
    betray a compressed container. None = unmeasurable."""
    if not data:
        return None
    if len(data) >= 12 and data[:4] == b"RIFF" and data[8:12] == b"WAVE":
        p = _parse_wav(data)
        if p:
            sr, ch, bits, pcm = p
            rate = sr * max(1, ch) * max(8, bits) // 8
            return (len(pcm) / rate) if rate > 0 else None
        return None
    ext = os.path.splitext(filename or "")[1].lstrip(".").lower()
    if (_base_ct(content_type) in _PCM_CONTENT_TYPES and ext in ("", "pcm", "raw")
            and not _is_compressed_audio(data)):
        return len(data) / (_TTS_PCM_SR * 2)   # 16-bit mono
    return None


def _pcm_bytes_duration_s(nbytes: int, content_type: str) -> Optional[float]:
    """Duration of a STREAMED TTS response from its byte count — the relay never holds the
    whole body (and its tee is capped), so there's no header to parse at the end. Valid
    because the engine streams 16-bit mono PCM at _TTS_PCM_SR, optionally behind the
    44-byte WAV header it emits up front. None for anything else."""
    ct = _base_ct(content_type)
    if ct in _WAV_CONTENT_TYPES:
        nbytes -= 44
    elif ct not in _PCM_CONTENT_TYPES:
        return None
    return (nbytes / (_TTS_PCM_SR * 2)) if nbytes > 0 else None


def _record_rtf(endpoint_id: str, model: str, kind: str, audio_s: Optional[float],
                proc_s: float) -> dict:
    """Record this request's RTF to Prometheus and return the fields the response headers
    are built from (empty when the duration was unmeasurable). `kind` is "stt"/"tts"."""
    if audio_s is None or audio_s <= 0 or proc_s <= 0:
        return {}
    from . import metrics as _metrics
    _metrics.observe_audio_rtf(endpoint_id, model, kind, audio_s, proc_s)
    return {"audio_seconds": audio_s, "rtf": proc_s / audio_s, "rtfx": audio_s / proc_s}


def _rtf_headers(res: dict) -> dict:
    """The RTF response headers for a forward result: `X-Audio-Seconds` (measured duration),
    `X-RTF` (processing seconds per audio second, lower = faster) and `X-RTFX` (its
    reciprocal — the "× real time" speed-up). Empty when no RTF was measured."""
    rtf = res.get("rtf")
    if rtf is None:
        return {}
    return {"X-Audio-Seconds": f"{res['audio_seconds']:.3f}",
            "X-RTF": f"{rtf:.4f}", "X-RTFX": f"{res['rtfx']:.2f}"}


def _body_duration_s(body: object) -> Optional[float]:
    """The `duration` an STT upstream reports (whisper verbose_json), if usable."""
...
async def _do_speech(app, endpoint_id: str, candidates: list[dict], alias: str,
    ...
    for u in candidates:
        ...
        # RTF over the audio we just generated (header-parsed; PCM/WAV only).
        res.update(_record_rtf(endpoint_id, alias, "tts",
                               _audio_duration_s(audio, ct), lat / 1000.0))
        res["raw_body"] = audio
        res["media_type"] = ct
        return res
    raise HTTPException(status_code=502, detail={"error": f"all upstreams failed: {last_err}"})
...
async def _stream_speech(app, request_id: str, endpoint_id: str, candidates: list[dict], alias: str,
    ...
    try:
        ...
        async def _relay():
            ...
            finished = False
            truncated = False
            nbytes = 0            # every relayed byte (the tee below is capped) → RTF
            buf = bytearray()
            try:
                async for chunk in r.aiter_bytes():
                    if cancel_ev.is_set():
                ...
                await _finish(request_id, "completed", status_code=r.status_code,
                              latency_ms=int(elapsed * 1000), ttft_ms=ttft, upstream=u["name"])
                # RTF from the streamed byte count. Metric only — the response headers
                # flushed before the first chunk, so a streamed TTS carries no X-RTF.
                _record_rtf(endpoint_id, alias, "tts", _pcm_bytes_duration_s(nbytes, ct), elapsed)
                if truncated:
                    from . import metrics as _metrics
                    _metrics.observe_tts_eval_outcome(endpoint_id, "skipped")
                elif buf:
    ...
    try:
        ...
        if "text/event-stream" not in r.headers.get("content-type", ""):
            ...
            await _finish(request_id, "completed" if ok else "failed", status_code=r.status_code,
                          ...
                          error=None if ok else (json.dumps(obj)[:500] if isinstance(obj, dict) else str(obj)[:500]))
            live.pop(request_id, None)
            if ok:  # a real (buffered) transcription → report its RTF like the unary path
                dur = _body_duration_s(obj) or _audio_duration_s(file_bytes, content_type, file_name)
                base_hdrs.update(_rtf_headers(_record_rtf(endpoint_id, alias, "stt", dur, lat / 1000.0)))
            return JSONResponse(obj, status_code=r.status_code, headers=base_hdrs)
        ...
        async def _relay():
            nonlocal token, gate_held
            ttft = None; lp = None; buf = ""; finished = False; dur = None
            try:
                async for chunk in r.aiter_bytes():
                    if cancel_ev.is_set():
                        break
                    ...
                    # the JSON parser off the content deltas). Most whisper streams emit
                    # neither; harmless if so.
                    try:
                        buf += chunk.decode("utf-8", "ignore")
                        while "\n\n" in buf:
                            frame, buf = buf.split("\n\n", 1)
                            if not any(k in frame for k in ("logprob", "segments", "duration")):
                                continue
                            for ln in frame.split("\n"):
                                if not ln.startswith("data:"):
                                    continue
                                ...
                                try:
                                    obj = json.loads(d)
                                    got = _extract_audio_logprob(obj)
                                    if got is not None:
                                        lp = got
                                    got_dur = _body_duration_s(obj)
                ...
                # RTF — the streamed frames rarely carry a `duration`, so fall back to the
                # clip we uploaded. Metric only: the SSE headers flushed before frame one.
                _record_rtf(endpoint_id, alias, "stt",
                            dur or _audio_duration_s(file_bytes, content_type, file_name), elapsed)
            finally:
                try:
                    await r.aclose()
                except Exception:
