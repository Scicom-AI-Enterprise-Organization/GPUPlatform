...
def test_fused_experts_dora():
    ...
    # (a) at init (B=0, mag=‖W‖) DoRA reduces to the pure base MoE.
    ...
    # (b) non-trivial adapter: fused == composed DoRA reference.
    with torch.no_grad():
        experts.gate_up_lora_b.normal_(std=0.5)
        experts.down_lora_b.normal_(std=0.5)
    ...
    # (c) gradients flow to the adapters AND magnitudes; the frozen base gets none.
    g = torch.randn(T, H)
