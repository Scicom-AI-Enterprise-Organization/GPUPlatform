...
const EXPORT_KIND = "gpuplatform.benchmark.export";

type ParsedExport = {
  kind?: string;
  source_bench_id?: string;
  benchmark?: { name?: string; status?: string; result_json?: unknown };
  files?: unknown[];
...
export function ImportBenchmarkForm() {
  ...
  function addFiles(files: FileList | File[]) {
    ...
    if (list.length === 0) {
      toast.error("Drop .json benchmark export files");
      return;
    }
    list.forEach((f, i) => {
      ...
      reader.onload = () => {
        ...
        try {
          ...
          if (obj?.kind !== EXPORT_KIND) {
            parseError = "Not a benchmark export (wrong `kind`).";
          } else {
            parsed = obj;
            name = obj.benchmark?.name ?? "";
          }
        } catch (e) {
  ...
  async function onImportAll() {
    ...
    let ok = 0;
    let lastId = "";
    // Snapshot the keys to import; mutate status per-key as we go.
    for (const target of importable) {
      ...
      try {
        const body = {
          ...target.parsed,
          benchmark: {
            ...target.parsed!.benchmark,
            name: target.name.trim() || target.parsed!.benchmark?.name || "imported",
          },
        };
        const rec = await gateway.importBenchmark(body);
        ok += 1;
        lastId = rec.id;
        setEntries((prev) =>
    ...
    setImporting(false);
    if (ok > 0) {
      toast.success(`Imported ${ok} of ${importable.length} benchmark${importable.length === 1 ? "" : "s"}`, {
        duration: 3500,
      });
      // All succeeded: jump to the single run, or the list for a batch. Any
      // failures → stay so the per-file errors remain visible.
      if (ok === importable.length) {
        if (ok === 1 && lastId) router.push(`/benchmark/${lastId}`);
  ...
  return (
    <div className="space-y-5">
      <div
        ...
        onClick={() => document.getElementById("bench-import-file")?.click()}
        role="button"
        tabIndex={0}
        className={cn(
      ...
      >
        ...
        <span className="text-sm font-medium">
          Drag &amp; drop <span className="font-mono text-xs">.benchmark.json</span> files here, or click to choose
        </span>
        <span className="text-xs text-muted-foreground">
          Multiple files supported — exported from a benchmark&apos;s Files → Export.
        </span>
        <input
          id="bench-import-file"
          type="file"
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {e.parsed && (
                    <div className="mt-1 text-[11px] text-muted-foreground">
                      ...
                      {e.parsed.benchmark?.result_json ? " · has results" : " · no results"}
                      {e.parsed.source_bench_id ? ` · from ${e.parsed.source_bench_id}` : ""}
                    </div>
                  )}
                </div>
                <button
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        <Button onClick={onImportAll} disabled={importing || importable.length === 0}>
          ...
          {importing
            ? "Importing…"
            : importable.length > 1
              ? `Import ${importable.length} benchmarks`
              : "Import benchmark"}
        </Button>
      </div>
    </div>
