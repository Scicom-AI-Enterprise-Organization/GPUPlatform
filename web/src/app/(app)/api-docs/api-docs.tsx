...
vllm:gpu_cache_usage_perc{app_id="my-endpoint",model="…"} 0.41`,
  ...
  // ───── Autotrain ─────
  {
    ...
    parameters: [
      ...
      { name: "augment_techniques / augment_prob", in: "body", type: "string[] / number", doc: "ASR training-audio augmentation at a probability. Classic: telephone/noise/dropout/gain/pitch/speed/reverb/bandpass. LiveKit/WebRTC voice-agent transport: livekit (the full chain — capture APM → Opus → packet loss/PLC → DTX → agent AGC → 48k/24k/16k → VAD), livekit_sip (G.711 trunk + tandem Opus), or the individual stages opus/g711/packet_loss/dtx/agc/webrtc_ns/aec/vad_clip/resample_chain. Streaming regime (benchmarked as the larger cost — the VAD segment and a speaker who pauses mid-turn): livekit_stream (transport chain + all of the following; start here for a human-facing agent), hesitation, vad_pad, rampup, denoiser, room_tone, jitter. Default [] / 0.5." },
