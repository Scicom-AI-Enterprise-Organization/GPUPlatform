#!/usr/bin/env bash
# Mistral-Small-4-119B (FP8 MoE + MLA) LoRA finetune — 4x H100 SXM on RunPod.
#
...
# (native mistral4 + mistral3 + FP8 + fused-experts), liger-kernel, kernels, ChiniDataset. Runs
# the LoRA correctness test (CPU), builds/downloads the packed dataset, then torchrun.
#
#   export HF_TOKEN=hf_...                  # gated download (canonical: autotrain/.env)
#   export WANDB_API_KEY=...                # only if you pass --wandb
#   MISTRAL_DEPS_ONLY=1 bash run.sh         # install + LoRA correctness test only (cheap smoke)
#   bash run.sh -- --max_steps 20 --limit_samples 4   # short run -> a LoRA checkpoint
...
MODEL_ID="${MODEL_ID:-mistralai/Mistral-Small-4-119B-2603}"
...
MISTRAL_DEPS_ONLY="${MISTRAL_DEPS_ONLY:-0}"
...
# ---------- remaining deps ----------
$PIP -U "transformers==5.5.0"   # native mistral4/mistral3 + FP8 (FineGrainedFP8) + fused-experts (grouped_mm)
$PIP liger-kernel               # LigerFusedLinearCrossEntropyLoss (avoids the 131k-vocab logits)
...
$PIP "kernels>=0.12.0,<0.13"    # finegrained-fp8 hub kernels (inference only; training dequants)
$PIP accelerate                 # device_map="auto" for compare/merge + low_cpu_mem_usage load
...
echo ">> LoRA correctness test (fused grouped-MoE math + grads, per-tensor + block dequant)"
MISTRAL_GROUPED_FALLBACK=1 python test_lora.py

if [ "$MISTRAL_DEPS_ONLY" = "1" ]; then echo ">> MISTRAL_DEPS_ONLY=1 — install + test done, skipping train"; exit 0; fi
...
# ~119GB FP8 across 3 HF-format shards (model-*.safetensors). EXCLUDE the mistral-common
# `consolidated-*` shards (a duplicate copy in a different format) + other redundant formats.
hf download "$MODEL_ID" --exclude "consolidated*" "original/*" "*.pth" "*.gguf"

# ---------- train ----------
echo ">> launching training on $NPROC GPUs"
torchrun --nproc_per_node="$NPROC" mistral_small.py "$@"
