"""Fold a trained MiniMax-M2 LoRA into a plain **bf16** MiniMaxM2ForCausalLM for vLLM serving.

Sibling of `../mistral-small/merge_to_bf16.py`. vLLM 0.23.0 supports `MiniMaxM2ForCausalLM`, but
can't apply our custom fused routed-expert LoRA as an adapter, so we fold everything into the
weights and save a standard bf16 checkpoint (no quantization_config), then vLLM serves it.
  ...
  * everything else (router gate, norms, embeddings, lm_head) -> bf16 copy

MiniMax-M2 differs from mistral-small: 128x128 BLOCK-scaled FP8 (dequantize_fp8_blockwise),
256 experts, NO shared experts, NO vision tower (text-only MiniMaxM2ForCausalLM). ~460GB on disk.

    python merge_to_bf16.py --lora checkpointing_64k/lora.pt --out /share/merged-minimax-64k
"""
import argparse
import copy
import os
...
@torch.no_grad()
def build_base_bf16_state_dict(model):
    """Dequant the FROZEN FP8 base to bf16 with NO LoRA (for serving the BASE model on vLLM, which
    chokes on MiniMax-M2's block-FP8 MoE dims). Mirror of build_merged_state_dict minus the fold."""
    merged = {}
    handled = []
    for name, module in model.named_modules():
        w = getattr(module, "weight", None)
        # bare FP8Linear (q/k/v/o, etc.): dequant weight*scale -> bf16
        if isinstance(w, torch.Tensor) and w.element_size() == 1 and getattr(module, "weight_scale_inv", None) is not None:
            merged[f"{name}.weight"] = dequantize_fp8_blockwise(
                w, module.weight_scale_inv, getattr(module, "block_size", None), out_dtype=torch.bfloat16)
            if getattr(module, "bias", None) is not None:
                merged[f"{name}.bias"] = module.bias.to(torch.bfloat16)
            handled.append(name + ".")
        # bare FP8Experts: dequant the 3D expert weights
            ...
            merged[f"{name}.gate_up_proj"] = dequantize_fp8_blockwise(
                module.gate_up_proj, module.gate_up_proj_scale_inv, bs, out_dtype=torch.bfloat16)
            merged[f"{name}.down_proj"] = dequantize_fp8_blockwise(
                module.down_proj, module.down_proj_scale_inv, bs, out_dtype=torch.bfloat16)
            handled.append(name + ".")
...
@torch.no_grad()
def build_merged_state_dict(model):
    merged = {}
    handled = []
    for name, module in model.named_modules():
    ...
    # misses these, but the clean bf16 model expects them in its state_dict.
    for bname, b in model.named_buffers():
...
def main():
    ...
    ap.add_argument("--out", default="/share/merged-minimax-64k")
    ap.add_argument("--no-lora", action="store_true", help="just dequant the FP8 base to bf16 (no LoRA fold) — for serving the BASE model")
    args = ap.parse_args()
    ...
    if args.no_lora:
        print(">> dequant base FP8 -> bf16 (no LoRA)", flush=True)
        merged = build_base_bf16_state_dict(model)
    else:
        merge_infer.attach_lora(model, args.lora, merge_infer.load_meta(args.lora))
        print(">> folding LoRA -> bf16 state_dict", flush=True)
        merged = build_merged_state_dict(model)
    del model
